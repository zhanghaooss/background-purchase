<template>
  <div>
    ...
    <div style="text-align: center; margin:10px 0px; font-size: 12px;">
      2022 © 后台采购管理系统-V1.0
    </div>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      loginRules: {
        checkName: [{ required: true, message: "请输入账号", trigger: "blur" }],
    ...
    handleSubmit() {
      ...
      this.$refs.loginData.validate(valid => {
        if (valid) {
          ...
          let loginParms = {
            parms: {
              checkName: this.loginData.checkName,
              checkPwd: this.loginData.checkPass
            }
          };
          _this
            .$httpGet("Login/UserLogin", loginParms)
            .then(res => {
              if (res.loginStatus) {
                //登录信息
                ...
                if (_this.$router.options.routes.length <= 1) {
                  ...
                  let twfxMenu = [];
                  //一级菜单
                  let menuOneList = _this.Enumerable.from(menuList)
                    .where(p => p.PARENTMENUID == null || p.PARENTMENUID == 0)
                    .orderBy(o => o.MENUSORT)
                    .toArray();
                  //二级菜单
                  let menuTwoList = _this.Enumerable.from(menuList)
                    .where(p => p.PARENTMENUID != null && p.PARENTMENUID != 0)
                    .toArray();
                  //循环
                  for (let i = 0; i < menuOneList.length; i++) {
                    ...
                    //二级菜单
                    let child = [];
                    let curTowList = _this.Enumerable.from(menuTwoList)
                      .where(p => p.PARENTMENUID == curMenu.MENUID)
                      .orderBy(o => o.MENUSORT)
                      .toArray();
                    ...
                    let menu = {
                      ...
                      component: resolve =>
                        require(["@/components/MasterPage.vue"], resolve),
                      children: child
                    };
                    twfxMenu.push(menu);
                  }
                ...
                if (existMenu != null) sessionStorage.removeItem("twfxmenu");
                let existUser = sessionStorage.getItem("twfxuser");
                if (existUser != null) sessionStorage.removeItem("twfxuser");
                ...
                if (existLg != null) localStorage.removeItem("loginName");
                localStorage.setItem("loginName", _this.loginData.checkName);
                _this.$router.push({ path: "/Home" });
              } else {
                if (
                  res.loginMessage != "undefined" &&
                  res.loginMessage != null &&
                  res.loginMessage != ""
                )
